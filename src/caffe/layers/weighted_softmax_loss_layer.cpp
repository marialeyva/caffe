...
void WeightedSoftmaxWithLossLayer<Dtype>::LayerSetUp(
  ...
  string weights_file = this->layer_param_.softmax_param().weights_file();
  char *weights_file_chr = new char[weights_file.length() + 1];
  strcpy(weights_file_chr, weights_file.c_str());
   std::ifstream src(weights_file_chr); {
		string name;
		...
		while (getline(src, name, ',')){
			...
			label_weights.push_back(strtof((name).c_str(),0));
                        aux++;
                 }
 
  LOG(INFO) << label_weights.size()<<" labels weights succesfully read from " << weights_file;
...
void WeightedSoftmaxWithLossLayer<Dtype>::Reshape(
  ...
  LOG(INFO) << outer_num_<<"  out " << inner_num_;
...
void WeightedSoftmaxWithLossLayer<Dtype>::Forward_cpu(
  ...
  for (int i = 0; i < outer_num_; ++i) {
    for (int j = 0; j < inner_num_; j++) {
      ...
      //Weight of the GT label
      const float label_weight = label_weights[label_value];
      if (has_ignore_label_ && label_value == ignore_label_) {
        continue;
      }
      DCHECK_GE(label_value, 0);
      DCHECK_LT(label_value, prob_.shape(softmax_axis_));
      loss -= log(std::max(prob_data[i * dim + label_value * inner_num_ + j]*label_weight,
                           Dtype(FLT_MIN)));
      ++count;
    }
  }
